...
import { Settings, Save, Award, Languages } from "lucide-react";
...
import { fillCzechTranslations } from "../utils/fillCzechTranslations";
...
export function SettingsManager({ token }: SettingsManagerProps) {
  ...
  const handleFillCzechTranslations = async () => {
    if (!confirm('Opravdu chcete vyplnit všechna česká pole u všech projektů?\n\nToto přepíše existující české překlady původními hodnotami z kódu.')) {
      return;
    }

    setUpdatingTranslations(true);
    try {
      const result = await fillCzechTranslations(token, projectId, publicAnonKey);
      toast.success(`✅ České překlady aktualizovány! Upraveno: ${result.updated}, Nenalezeno: ${result.notFound}`);
    } catch (error) {
      console.error('Error filling Czech translations:', error);
      toast.error('❌ Chyba při aktualizaci českých překladů');
    } finally {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Vyplnit všechna česká pole u všech projektů původními hodnotami
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            <p className="text-sm text-muted-foreground mb-4">
              Tato funkce automaticky vyplní všechna česká pole (názvy, popisy, specifikace, kategorie atd.) 
              u všech projektů podle původních hardcoded hodnot z kódu.
            </p>
            <p className="text-sm text-muted-foreground mb-4">
              <strong>Upozornění:</strong> Toto přepíše všechny existující české překlady!
            </p>
            <Button 
              onClick={handleFillCzechTranslations} 
              disabled={updatingTranslations}
              variant="outline"
            >
              <Languages className="h-4 w-4 mr-2" />
              {updatingTranslations ? 'Aktualizuji...' : 'Vyplnit české překlady'}
            </Button>
          </div>
        </CardContent>
      </Card>
