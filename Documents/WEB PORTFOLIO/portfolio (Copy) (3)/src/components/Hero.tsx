import { ArrowDown, Mail, Award, Box, Instagram, Linkedin, Github, Twitter, Youtube, Palette, Video, Pin, Link as LinkIcon, Music, Heart } from "lucide-react";
...
export function Hero({ language }: HeroProps) {
  ...
  const getAchievementLink = (type: string, id: string) => {
    ...
    switch (type) {
      case 'achievement':
        // Check if this achievement is a certification
        if (Array.isArray(achievements) && achievements.length > 0) {
          const achievement = achievements.find(a => a.id === id);
          if (achievement && achievement.type === 'certification') {
