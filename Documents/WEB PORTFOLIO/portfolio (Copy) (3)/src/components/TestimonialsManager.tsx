...
import { Plus, Edit, Trash2, Star, MessageSquare, Eye, EyeOff, Languages } from "lucide-react";
...
export function TestimonialsManager({ token }: TestimonialsManagerProps) {
  ...
  const handleAutoTranslate = async (
    ...
    const sourceText = editingTestimonial[sourceField as keyof Testimonial];
    ...
    try {
      ...
      if (translated) {
        ...
        setEditingTestimonial({ ...editingTestimonial, [targetField]: translated });
  ...
  // Auto-fill Czech content from English
  const handleAutoFillCzechFromEnglish = async () => {
    if (!editingTestimonial) return;

    if (!editingTestimonial.content || editingTestimonial.contentCs) {
      toast.info('Česká verze už je vyplněná nebo chybí anglický text');
      return;
    }

    setTranslating(true);
    try {
      const translated = await translateText(editingTestimonial.content, token, 'en', 'cs');
      if (translated) {
        setEditingTestimonial({ ...editingTestimonial, contentCs: translated });
        toast.success('✅ Přeloženo!');
      } else {
        toast.error('Překlad selhal');
      }
  ...
  const emptyTestimonial: Testimonial = {
    ...
    company: 'Tech Company s.r.o.',
    content: 'Working with Filip was an excellent experience. His attention to detail and expertise in CAD design resulted in a high-quality 3D printed part that exceeded our expectations. Professional communication and timely delivery made the collaboration smooth and successful.',
    contentCs: 'Práce s Filipem byla výborná zkušenost. Jeho pozornost k detailu a odbornost v CAD designu vyústily ve vysoce kvalitní 3D tištěnou součástku, která předčila naše očekávání. Profesionální komunikace a včasné dodání učinily spolupráci hladkou a úspěšnou.',
    rating: 5,
    initials: 'JN',
  };
        ...
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <div className="flex items-center justify-between">
              ...
              {editingTestimonial && editingTestimonial.content && (
                <Button
                  type="button"
                  variant="outline"
                  size="sm"
          ...
          {editingTestimonial && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="flex items-center justify-between">
                    ...
                    <Button
                      ...
                      onClick={() => handleAutoTranslate('content', 'contentCs', 'en-cs')}
                      disabled={translating || !editingTestimonial.content}
                    >
                ...
                <div>
                  <Label className="flex items-center justify-between">
                    <span>Content (Czech)</span>
                    <Button
                      type="button"
                      variant="ghost"
                      size="sm"
                      className="h-6 px-2 text-xs"
                      onClick={() => handleAutoTranslate('contentCs', 'content', 'cs-en')}
                      disabled={translating || !editingTestimonial.contentCs}
                    >
                      <Languages className="h-3 w-3 mr-1" />
